<section id="filtros-generales">
                 ...
                 <?php
                        ...
                        while ($row2 = mysqli_fetch_assoc($result2)){$i++;
                            if($row2['_id_categoria'] != 0)
                            {
                                ...
                                $result222 = $db_Full->select_sql("SELECT icat.nombre_item_categoria,icat.url_page_tbl 
                                                                   FROM tbl_categorias 
                                ...
                                while ($row222 = mysqli_fetch_assoc($result222)){
                                    $data = array(
                                                  "nombre_item_filtro"     => $row222['nombre_item_categoria'],
                                                  "fk_item_filtro"         => $row2['id_filtro']
                                                 );

                                    array_push($lista_items , $data);
                                }
                                $result222 = $db_Full->select_sql("delete from tbl_items_filtro WHERE fk_item_filtro = ".$row2['id_filtro']);
                                $query = $db_Full->insert_table("tbl_items_filtro",$lista_items);
                            } 
                 ?>
                            ...
                            <ul class="dropdown-menu" aria-labelledby="filtroTalla">
                                ...
                                if(mysqli_num_rows($result3))
                                {
                                    while($row3 = mysqli_fetch_assoc($result3))
                                    {
                                        ...
                                        <li>
                                            <?php if(isset($id_cat)){?>
                                            <span data_link="link_row" data="<?php echo $row2["id_filtro"];?>" tipo = "<?php echo $id_tipo;?>" data2="<?php echo $row3["id_item_filtro"];?>" categoria="<?php echo  $id_cat;?>" marca="0"  subcategoria="<?php echo  $id_page_table;?>" cantidades="<?php echo $cantidad_paginador;?>" onclick="captar_filtro_recien_llegados(this);" onclick2="setCookie('filtro_<?php echo $i?>','<?php echo $row3["nombre_item_filtro"]; ?>','1')"><?php echo ucfirst(strtolower($row3["nombre_item_filtro"])); ?>
                                            </span>
                                            <?php }
                                            else{
                                            ?>   
                                                <span data_link="link_row" data="<?php echo $row2["id_filtro"];?>" tipo = "<?php echo $id_tipo;?>" data2="<?php echo $row3["id_item_filtro"];?>" categoria="" marca="0" subcategoria="" cantidades="<?php echo $cantidad_paginador;?>" onclick="captar_filtro_recien_llegados(this);" onclick2="setCookie('filtro_<?php echo $i?>','<?php echo $row3["nombre_item_filtro"]; ?>','1')"><?php echo ucfirst(strtolower($row3["nombre_item_filtro"])); ?>
                                            </span>
                                            <?php }?> 
                                        </li>
   ...
   inner join tbl_tipos_marcas tmar on fkmarcas_tipos_marcas =id_marca and fktipos_tipos_marcas =".$id_tipo." inner join tbl_productos_marcas pm on pm.fkmarca_productos_marcas =id_marca and pm.fktipo_productos_marcas=".$id_tipo." group by id_marca"); 
                                                          ...
                                                          ?>
                                                            <li>
                                                                <span href="<?php echo $BASE_URL.$url3;?>" data_link="link_row" tipo = "<?php echo $id_tipo;?>" data="<?php echo $row2["id_filtro"];?>" categoria="<?php echo  $id_cat;?>" subcategoria="<?php echo  $id_page_table;?>" cantidades="<?php echo $cantidad_paginador;?>" subcategoria="<?php echo  $id_page_table;?>" data2="<?php echo $row3["id_item_filtro"];?>" marca="<?php echo $row_subf["id_marca"]; ?>"  onclick="captar_filtro_recien_llegados(this);"><?php echo ucfirst(strtolower($row_subf["nombre_marca"])); ?>
   ...
   inner join tbl_tipos_marcas tmar on fkmarcas_tipos_marcas =id_marca and fktipos_tipos_marcas =".$id_tipo." inner join tbl_productos_marcas pm on pm.fkmarca_productos_marcas =id_marca and pm.fktipo_productos_marcas=".$id_tipo." group by id_marca"); 
                                          ...
                                          ?>
                                            <li>
                                                <span href="<?php echo $BASE_URL.$url3;?>" data_link="link_row" tipo="<?php echo $id_tipo;?>" data="<?php echo $row2["id_filtro"];?>" cantidades="<?php echo $cantidad_paginador;?>" categoria="<?php echo  $id_cat;?>" subcategoria="<?php echo  $id_page_table;?>" subcategoria="<?php echo  $id_page_table;?>" data2="<?php echo $row3["id_item_filtro"];?>" marca="<?php echo $row_subf["id_marca"]; ?>"  onclick="captar_filtro_recien_llegados(this);"><?php echo strtolower($row_subf["nombre_marca"]); ?>
